...
def load_race(data_path=None,name='middle', pondering=None, keys_path=None):
    ...
    if data_path:
        dataset = load_dataset(data_path, name =name,trust_remote_code=True)
    else:
        
        dataset = load_dataset('ehovy/race', name =name,trust_remote_code=True)
    data = dataset["test"]
    ...
    for idx, item in enumerate(data):
        ...
        if question[-6:] == '  _  .':
            formatted_item = {
                "prefix": 'Article: '+context + question[:-5],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subset_name", type=str, default='middle')
    ...
    list_data_dict = load_race(args.data_path,args.subset_name)
    ...
    if args.pondering is not None:
        list_data_dict_keys = load_race(args.data_path,args.subset_name,pondering=args.pondering, keys_path=args.keys_path)
