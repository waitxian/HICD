...
def load_openbookqa(pondering=None, keys_path=None):
    ...
    for idx, item in enumerate(data):
        ...
        if pondering == "hard" and keys_path is not None:
            formatted_item["prefix"] = f"Pondering: {key_words[idx % len(key_words)]}\n\nContext: {formatted_item['prefix']}"
...
def generate_layer_pair_configs(config_list):
    layer_combinations = combinations(config_list.keys(), 2)
    result = []
    
    for layer1, layer2 in layer_combinations:
        head_config = {
            layer1: config_list[layer1],
            layer2: config_list[layer2]
        }
        result.append(head_config)
...
if __name__ == "__main__":
    ...
    list_data_dict = load_openbookqa()
    ...
    if args.pondering is not None:
        list_data_dict_keys = load_openbookqa(pondering=args.pondering, keys_path=args.keys_path)
